...
EOSQL
    ...
    if [ "$MYSQL_USER" -a "$MYSQL_PASSWORD" ]; then
      echo "DELETE FROM mysql.user WHERE user='$MYSQL_USER';" >> "$tempSqlFile"
    ...
    # this is the Single State Transfer user (SST, initial dump or xtrabackup user)
      ...
      echo "DELETE FROM mysql.user WHERE user='${WSREP_SST_USER}';" >> "$tempSqlFile"
      ...
      # add superadmin
      echo "DELETE FROM mysql.user WHERE user='superadmin';" >> "$tempSqlFile"
      echo "CREATE USER 'superadmin'@'localhost' IDENTIFIED BY '${MYSQL_ROOT_PASSWORD}';" >> "$tempSqlFile"
      echo "GRANT ALL ON *.* TO 'superadmin'@'localhost';" >> "$tempSqlFile"
      echo 'FLUSH PRIVILEGES ;' >> "$tempSqlFile"
    fi
